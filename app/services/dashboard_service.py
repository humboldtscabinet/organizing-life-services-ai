...
# Non-actionable storefront URLs whose GSC traffic must not inflate keyword tasks.
# Match path prefixes after URL parsing so full GSC page URLs are covered.
...
def _normalize_page_path(page: str) -> str:
    """Extract a lowercase path from a full or relative GSC page URL."""
...
def _is_seo_denylisted_page(page: Optional[str]) -> bool:
    """
    Return True if a GSC page URL is on the SEO denylist.

    Denylisted URLs (Shopify products, cart, utility collections, etc.) are
    not actionable keyword-optimization targets. Their impressions/clicks must
    be excluded from Rules 1–3 aggregates, not merely used to skip a query when
    *every* page for that query is denylisted.
    """
    if not page:
        return False

    path = _normalize_page_path(page)

    for prefix in SEO_DENYLIST_PATH_PREFIXES:
...
def _generate_gsc_tasks(db: Session, cutoff: datetime) -> list[dict]:
    ...
    # Aggregate by query — exclude denylisted page rows so product/utility
    # traffic cannot inflate impression thresholds or CTR/position averages.
    query_stats = {}
    for r in records:
        if _is_seo_denylisted_page(r.page):
            continue
